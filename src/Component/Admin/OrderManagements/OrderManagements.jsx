...
const OrderManagement = () => {
  ...
  const getDefaultData = () => ({
    ...
    deliveryPartners: [
      { id: "deliveryPartner1", name: "Delivery Partner 1" },
      { id: "deliveryPartner2", name: "Delivery Partner 2" },
      { id: "deliveryPartner3", name: "Delivery Partner 3" },
    ],
  });
  ...
  useEffect(() => {
    const saveData = {
      ...
      deliveryPartners: dashboardData.deliveryPartners,
    };
  ...
  const filteredOrders = dashboardData.recentOrders.filter((order) => {
    const platformMatch =
      dashboardData.filters.platform === "all" ||
      order.platform === dashboardData.filters.platform;
    const statusMatch =
      dashboardData.filters.status === "all" ||
  ...
  const deleteOrder = (orderId) => {
    if (window.confirm("Are you sure you want to delete this order?")) {
      setDashboardData((prev) => ({
        ...prev,
        recentOrders: prev.recentOrders.filter(
  ...
  const assignToPartner = (orderId, partnerId) => {
    const partner = dashboardData.deliveryPartners.find(
      (p) => p.id === partnerId
    );
    ...
    setDashboardData((prev) => ({
      ...
      recentOrders: prev.recentOrders.map((order) =>
        order.orderId === orderId
          ? { ...order, assignedPartner: partner.name }
          : order
      ),
    }));
  };

  // Unassign partner from order
  const unassignPartner = (orderId) => {
    setDashboardData((prev) => ({
      ...prev,
      recentOrders: prev.recentOrders.map((order) =>
        order.orderId === orderId ? { ...order, assignedPartner: "" } : order
      ),
    }));
  };
      ...
      <div className="row">
        ...
        <div className="col-12 mb-4">
          ...
          <p className="text-muted mb-0">
            All orders (from all platforms) appear in one table
          </p>
        </div>
                          ...
                          <td>
                            <span
                              className={`badge ${getStatusBadgeClass(
                                order.status
                              )} px-3 py-1 rounded-pill fw-medium`}
                            >
                              {order.status}
                            </span>
                          </td>
                          ...
                          <td>
                            <div className="dropdown">
                              <button
                                ...
                                aria-expanded="false"
                              >
                                {order.assignedPartner || "Assign Partner"}
                              </button>
                              <ul
                                className="dropdown-menu"
                                aria-labelledby={`assignDropdown-${order.orderId}`}
                              >
                                <li>
                                ...
                                {deliveryPartners.map((partner) => (
                                  <li key={partner.id}>
                                    <button
                                      ...
                                      onClick={() =>
                                        assignToPartner(order.orderId, partner.id)
                                      }
                                    >
            ...
            {filteredOrders.length > 0 && (
              <div className="card-footer bg-white border-0 d-flex justify-content-between align-items-center">
                <div className="text-muted">
                  Showing{" "}
                  <span className="fw-medium">1-{filteredOrders.length}</span>{" "}
                  of <span className="fw-medium">{filteredOrders.length}</span>{" "}
                  orders
                </div>
                <nav>
              ...
              <div className="modal-body">
                <div className="row">
                  <div className="col-md-6 mb-3">
                    ...
                    {viewOrder.assignedPartner && (
                      <div className="mb-2">
                        <span className="fw-medium">Assigned To:</span>{" "}
                        {viewOrder.assignedPartner}
                      </div>
                    )}
                  </div>
